...
└─────────────────┘     │  /message (чат)  │────▶│  /rag/context — Chroma      │
                          └────────┬────────┘     └─────────────────────────────┘
                                   │
...
├── rag/              # Chroma, retrieval (библиотека)
├── data/             # .txt статьи для RAG (apple: 16 статей)
...
После смены статей: `docker compose exec classifier python scripts/reindex_rag.py` и `docker compose restart classifier`.
