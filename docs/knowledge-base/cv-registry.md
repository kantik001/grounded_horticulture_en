...
If variable missing → `None` → classifier without your weights (ImageNet backbone only).
...
| file missing | `create_classifier()` without weights + log `No weights — ImageNet backbone only` |
...
flowchart TD
    ...
    I -->|no| K[create_classifier no weights]
    J --> L[cache + return]
    K --> L
...
1. Check log: `Loading weights` vs `No weights — ImageNet backbone only`.
