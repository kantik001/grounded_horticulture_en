...
### Important details (not like “big” DevOps)

1. **No `schema_migrations` table** — project **does not record** which files were already applied.
2. On **every startup** server runs **all** `.sql` files in alphabetical order (`001` → `002` → `003`).
3. Therefore **`IF NOT EXISTS`** / **`ADD COLUMN IF NOT EXISTS`** everywhere — repeat run does not fail.

Simpler for learning; for production with many migrations you usually add version tracking (Flyway, golang-migrate). With 3 files — schema works.
...
Migrations are **versioned DB schema in SQL**. Go applies them on every startup, safely via `IF NOT EXISTS`. Understanding `CREATE`, `ALTER`, `REFERENCES`, `CHECK`, `INDEX` — base for reading any new `00N_*.sql`.
