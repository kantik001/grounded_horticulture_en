# Разбор: `classifier/apple_classifier.py`

**Исходный файл:** `classifier/apple_classifier.py`  
**Язык:** Python (PyTorch + torchvision)  
**Связанные модули:** `classifier/registry.py`, `classifier/api_server.py`, `classifier/train_classifier.py`  
**Кто вызывает:** `registry.get_classifier_for_crop()` → `api_server.py` (`POST /classify`)
...
Файл **не поднимает HTTP** — только логика модели. HTTP — в `api_server.py`.

---

## Классы, которые распознаёт модель

Константа `CLASS_LABELS` (порядок важен: индекс 0 = первый класс, и т.д.):
...
При обучении (`train_classifier.py`) порядок классов в датасете должен **совпадать** с этим списком (или сохраняться в checkpoint — см. ниже).
...
2. **`self.model = self._load_model(model_path)`** — собирает и загружает веса.
3. **`self.model.eval()`** — режим инференса (без dropout на обучении, фиксированное поведение).
4. **`self.transform`** — пайплайн подготовки картинки:
...
## Загрузка модели: `_load_model` (строки 56–81)
...
Важно: в `registry.py` путь к весам задаётся через `.env` (`MODEL_PATH`, `MODEL_PATH_APPLE`). Если файла нет — в лог пишется «ImageNet backbone only», обученная голова не подгружается.
...
```mermaid
flowchart LR
    A[api_server POST /classify] --> B[registry.get_classifier_for_crop]
...
- `generateRecommendation` / `generateTemplateRecommendation` — текст совета по классу;
...
Подробный разбор обучения — [classifier-train_classifier.md](./classifier-train_classifier.md).
...
Скорее всего нет вашего `.pth` или мало данных при обучении. Проверьте лог registry: `Loading model from ...` vs `No weights — ImageNet backbone only`.

### Дублирование `predict` и `predict_from_bytes`

Исторически: файл — для скриптов, bytes — для API. Логику можно было вынести в один приватный метод `_predict_tensor` — сейчас код продублирован намеренно просто.
...
| Тема | Файл |
|------|------|
| HTTP и эндпоинт `/classify` | [classifier-api_server.md](./classifier-api_server.md) |
| Выбор модели и кэш | [classifier-registry.md](./classifier-registry.md) |
| Обучение весов | [classifier-train_classifier.md](./classifier-train_classifier.md) |
| Рекомендация пользователю после CV | `server/photo_recommendations.go` (`generateRecommendation`) |

---

## Краткий итог

`apple_classifier.py` — **ядро Computer Vision**: загрузка MobileNetV2, подмена головы на 10 классов, препроцессинг, inference, JSON-результат. В проде вызывается только через **`predict_from_bytes`** из `api_server.py`; Go получает готовые `prediction` и `confidence` и дополняет ответ текстом.
