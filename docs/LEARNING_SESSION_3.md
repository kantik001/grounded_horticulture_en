...
4. **RAG** — гибридный поиск (Chroma + BM25 + reranker) с фильтром `crop_id` (`rag/vector_store.py`, см. `docs/knowledge-base/rag-hybrid-search.md`).
...
| **Metadata в индексах** | `crop_id` + `chunk_id` в каждом чанке → фильтр и RRF при hybrid search |
